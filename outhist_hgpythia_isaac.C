...
TLegend* lg_compile=new TLegend(0.16,0.74,0.42,0.853);
lg_compile->AddEntry(integrated_raa[0],"Ru+Ru");
lg_compile->AddEntry(integrated_raa[1],"Zr+Zr");
lg_compile->SetBorderSize(0);

TLegend* lg_pperr=new TLegend(0.16,0.853,0.42,0.91);
lg_pperr->AddEntry(pp_errbox,"pp uncertainty","f");
lg_pperr->SetBorderSize(0);
lg_pperr->Draw();
...
TLegend* lg_compare=new TLegend(0.42,0.845,0.66,0.91);
lg_compare->AddEntry(mod_auau,"Au+Au");
TLegend* lg_compare_dau=new TLegend(0.42,0.727,0.66,0.792);
lg_compare_dau->AddEntry(mod_dau,"d+Au");
TLegend* lg_compare_cu=new TLegend(0.42,0.8,0.66,0.845);
lg_compare_cu->AddEntry(pion_cucu,"Cu+Cu #frac{#pi^{+}+#pi^{-}}{2}");

lg_compare->SetBorderSize(0);
lg_compare_dau->SetBorderSize(0);
lg_compare_cu->SetBorderSize(0);
...
compile_raa->SaveAs(Form("prelim_plots/raa_summary_au_%ibin.pdf",ncent));
cin>>buf_char;

lg_compare_cu->Draw();
pion_cucu->Draw("samep");
compile_raa->SaveAs(Form("prelim_plots/raa_summary_cu_%ibin.pdf",ncent));
cin>>buf_char;
lg_compare_dau->Draw();
mod_dau->Draw("samep");
compile_raa->SaveAs(Form("prelim_plots/raa_summary_dau_%ibin.pdf",ncent));
cin>>buf_char;
...
lg_compile->Draw();
compile_raa->SaveAs(Form("prelim_plots/raa_summary_data_%ibin.pdf",ncent));
cin>>buf_char;
...
compile_raa->SaveAs(Form("prelim_plots/raa_summary_theory_%ibin.pdf",ncent));
cin>>buf_char;
